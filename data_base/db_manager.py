...
from settings import order_calculator
...
class DBManager(metaclass=Singleton):
    ...
    def select_all_products_category(self, category):
        result = self._session.query(Books).filter_by(
    ...
    def select_all_product_id(self):
        ...
        return order_calculator._convert(result)
    ...
    def select_single_product_quantity(self, rownum):
        result = self._session.query(
            Books.quantity).filter_by(id=rownum).one()
        self.close()
        return result.quantity

    def update_product_value(self, rownum, name, value):
        self._session.query(Books).filter_by(
    ...
    def select_single_product_name(self, rownum):
        result = self._session.query(Books.name).filter_by(id=rownum).one()
        self.close()
        return result.name

    def select_single_product_author(self, rownum):
        result = self._session.query(Books.author).filter_by(id=rownum).one()
        self.close()
        return result.author

    def select_single_product_price(self, rownum):
        result = self._session.query(Books.price).filter_by(id=rownum).one()
        self.close()
        return result.price
    ...
    def save_order_data(self, order_data):
        ...
        order_entry = Order(quantity=total_quantity, data=total_cost)

        self._session.add(order_entry)
        self._session.commit()
        self.close()